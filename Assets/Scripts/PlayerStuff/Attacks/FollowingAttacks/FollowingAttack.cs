...
public class FollowingAttack : Attack
{
    ...
    public override void Tick(NetworkObject player, int Direction = 0, bool skipCooldown = false)
    {
        ...
        FollowingAttackData.LevelData usedLevelData = levelData;

        foreach (ItemList i in items)
        {
            usedLevelData = i.item.FollowingAttackDataMod(player1, i.stacks, levelData); // need to add to rest
        }
        ...
        for (int i = 0; i < ((levelData.projCount + player1.additiveProjectileModifier) * player1.percentageProjectileSpeed); i++)
        {
            ...
            enemyNetworkObject.GetComponent<FollowingAttackProj>().Initialize(playerId, levelData.damage, levelData.speed, levelData.Area);
            enemyNetworkObject.GetComponent<FollowingAttackProj>().prefab = proj;
